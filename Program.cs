...
namespace hw6._6
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            while (isWork)
            {
                ...
                {
                    case "1":
                        player.ShowAllProduct(salesman);
                        break;
                    case "2":
                        player.BuyOperation(salesman, player);
                        break;
                    case "3":
                        player.ShowAllProduct(player);
                        break;
                    case "4":
                        isWork = false;
                        break;
    ...
    class Human
    {
        ...
        public void ShowAllProduct(Human human)
        {
            if(human.Inventory.Count > 0)
            {
                foreach (Product subject in human.Inventory)
                {
        ...
        private protected void TransferMoney(Salesman salesman, Player player, List<Product> inventory, int productId)
        {
        ...
        private protected Product DeleteProduct(int userInput, Salesman salesman)
        {
            Product product;
            product = salesman.Inventory[userInput];
            salesman.Inventory.RemoveAt(userInput);
            return product;
        }
    }
    ...
    class Player : Human
    {
        ...
        public void BuyOperation(Salesman salesman, Player player)
        {
            ...
            {
                idProduct = salesman.SerachIdProduct(salesman, result);

                if (idProduct >= 0)
                {
                    if (salesman.ExaminationPrice(player, salesman, idProduct))
                    {  
                        player.Inventory.Add(player.DeleteProduct(idProduct, salesman));
    ...
    class Salesman : Human
    {
        ...
        public int SerachIdProduct(Salesman salesman, int userInput)
        {
            int intermediateId = 0;
            int idProduct = -1;

            foreach (var product in salesman.Inventory)
            {
        ...
        public bool ExaminationPrice(Player player, Salesman salesman, int productId)
        {
            return salesman.Inventory[productId].Price <= player.Money;
        }
    }
